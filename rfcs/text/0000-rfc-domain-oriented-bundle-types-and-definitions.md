...
1. An mechanism to define a bundle types schema.

1. An method to specify the type of a bundle.
...
1. The bundle.json created by the HCA CLI download is renamed to manifest.json
 
1. Type information is added to a new bundle.json file rather than added to the DSS manifest.json file. 
...
1. The data_groups concept from [Processing Datasets that Span Multiple Data Collection Runs RFC](https://github.com/HumanCellAtlas/dcp-community/pull/88) is integrated and refined and corresponding bundle type is specified.

1. The bundle types schema is maintained new ```bundle_schema``` schema Github repository and documented on the Data Portal rather than maintained in a DSS registry.
...
Currently several bundle types in the HCA DSS that can be discriminated from each other by inspection of the scientific metadata files contained in the bundle. 

Roughly,

1. Primary bundles do not contain an analysis.json file.
...
## No schema makes validation harder
...
1. When a new pipeline or pipeline is added
1. When a DOI is available for a release.
1. When secondary analysis is available for a project version.
...
1. Bundle are currently not well documented and are not self documenting. 
...
1. “hca/expression-matrix” - a possible subtype representing the expression matrix generation.
...
_lifecycle_step_ - indicates the lifecyle state that is changing.
...
>TODO: Review the above with Mark D. 
...
1. Keeps types in the as bundle content and decoupled from the storage system implementation (out of the DSS CLI manifest.json, out of DSS bundle tags).

1. Refines the  "data_bundle" RFC 88 to link it to the scientific data model.
